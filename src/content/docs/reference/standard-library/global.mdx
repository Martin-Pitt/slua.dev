...
```
:::tip
[Luau supports generalized iterators](https://luau.org/syntax#generalized-iteration) as the modern and preferred method so you don't need to use the obsolete `ipairs` and `pairs` functions:
```slua
...
## Full reference
Most library functions are part of a library like `math` or `table`. However there are some that don't have any namespacing and are just global functions.
