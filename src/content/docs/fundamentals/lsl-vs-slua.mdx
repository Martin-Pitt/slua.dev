...
If you are an experienced LSL scripter and want to dive into SLua, then this guide by Suzanna Linn is for you.

You can check out the complete guide at [Suzanna’s SLua guide](https://suzanna-linn.github.io/slua/), which contains other information and example scripts as well.
...
These are the three basic types: `boolean` (`true` or `false`), `number` (which includes integers and floats) and `string`.
...
The increment and decrement operators, `++` and `--`, do not exist.

The compound operators, `+=` and so on, do not exist in standard Lua, but they exist in *Luau*, so SLua will also have them: `+=`, `-=`, `*=`, `/=`, `//=`, `%=`, `^=`.
...
We are not looking at events or states, because we don't know yet how they are implemented in SLua.
...
:::tip[Update]
Since this article was written SLua has added [event handling](../../fundamentals/events) via [`LLEvents`](../../reference/standard-library/llevents)
:::


<Attribution
	title="Suzanna’s SLua guide"
	author="2025 Suzanna Linn"
	source="https://suzanna-linn.github.io/slua/"
	license={{
